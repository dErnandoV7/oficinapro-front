...
import { Plus, RefreshCw, Search, MoreHorizontal, Eye, Pencil, Trash2, Users } from "lucide-react"
...
const ClientesPage = () => {
  ...
  useEffect(() => {
    ...
    const immediate = forceReloadRef.current
    forceReloadRef.current = false

    const timer = window.setTimeout(async () => {
      try {
        const res = await listClients(q ? { q } : undefined)
        if (ignore) return
        ...
        setLoadError(
          serverMessage.trim() || "Não foi possível carregar os clientes. Tente novamente.",
        )
        setClientes([])
      } finally {
        if (ignore) return
  ...
  const handleSalvarCliente = async (dados: {
    ...
    if (modoEdicao && clienteSelecionado) {
      const res = await updateClient(clienteSelecionado.id, {
        name: dados.nome,
      ...
      const updated = toClienteUi(res.data)
      setClientes((prev) => prev.map((c) => (c.id === updated.id ? updated : c)))
      setClienteSelecionado(updated)
      setModoEdicao(false)
    ...
    const created = toClienteUi(res.data)
